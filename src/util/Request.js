  ...
  response => {
    ...
    if (response.data && ["999999", "401", 401].indexOf(response.data.code) !== -1) {
      ...
      if (response.data && response.data.data) {
        ...
        window.location = window.Server.loginUrl;
      }
      return null;
...
export async function request (method, _url, params) {
  ...
  if (method.toLowerCase() === 'get') {
    return await getQuest(method, url, params);
  } else {
    return await postQuest(method, url, params);
  }
};
...
async function getQuest(method, url, param) {
  ...
  if (param) {
    ...
    if (_keys) {
      _url += '?';
      _keys.map(_key => {
        _url += `${_url.lastIndexOf('?') === _url.length -1 ? '' : '&'}${_key}=${param[_key]}`;
      });
      url = `${url}${_url}`;
    }
  }
